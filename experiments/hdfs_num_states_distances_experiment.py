...
# Setup directories
base_folder = get_experiments_path().joinpath("hdfs").joinpath("num_states")
            ...
            try:
                ...
                log_file = str(
                    base_folder.joinpath(
                        "logs",
                        f"distance_{max_sequence_length}_states{num_states}_levenshtein_{seed}.log",
                    )
                )
                dfa_file = str(
                    base_folder.joinpath(
                        "dfas",
                        f"distance_{max_sequence_length}_states{num_states}_levenshtein_{seed}",
                    )
                )
                log_file_gurobi = str(
                    base_folder.joinpath(
                        "gurobi_logs",
                        f"distance_{max_sequence_length}_states{num_states}_levenshtein_{seed}_gurobi.log",
                    )
                )
